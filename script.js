...
const todosLosTalles = document.querySelectorAll(".talles")

todosLosTalles.forEach(seleccionDeTalle => {
...
function selectorDeTalle(event){
    ...
    const talleElegido = selector.closest(".talles"); //Selecciono el elemento con clase ".talles" más cercano

    //PRUEBA BARRA TALLE, EL RESTO FUNCIONA

    const barraTalle = talleElegido.querySelector(".barra-talles"); //selecciono la barra talle más cercana para dejarla fija.

    barraTalle.classList.add("barra-talles--fijo")

    todosLosTalles.forEach(desEleccionDeTalle => {
        desEleccionDeTalle.addEventListener("click", borrarTalleElegido);
    });

    function borrarTalleElegido(event){
        
        barraTalle.classList.remove("barra-talles--fijo")
    }

    //PRUEBA BARRA TALLE, EL RESTO FUNCIONA

    talleProducto = talleElegido.textContent;//Obtengo el textContent del elemento con clase ".talles" más cercano

}

// 1bis) Creamos un array con todos los botones de compra
...
function sumarAlCarrito (nombreProducto, imagenProducto, precioProducto, talleProducto){ //Utilizamos lo obtenido en la función "agregarACarrito"
    ...
    <div class="productos-autogenerados">
        ...
        <div class="carrito-talle"><p class="texto-filas">${talleProducto}</p></div>
